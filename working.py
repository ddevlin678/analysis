...
def plottingGraph(tweetObject):
 range = pd.date_range(start, finish, freq='1min')
 tweetTime.groupby(df.index.map(lambda t: t.minute))
 #for x in range
 df = pd.DataFrame(index = range)
 global count1
 minutes=[]
 b = a + datetime.timedelta(minutes=count1)
 tweetTime = tweetObject[2]
 #tweetTime = tweetTime[-6:]
 print(start)
 #print(tweetTime)
 if start <= tweetTime <= finish:
   
  #today8am = now.replace(hour=8, minute=count1, second=0, microsecond=0)
  minutes.append(count1)
...
def inTweetAPos1(TeamA, tweetObject):
 #tweetObject = row[1]
 tweet = tweetObject#['new_tweet']
 print(type(tweet), tweet, "I AM IN POS A SENT2")
 for teamNames in TeamA:
  global TeamAPos
  global wordCloudPosA
  global TeamAPosLive
...
def inTweetBPos1(TeamB, tweetObject):
 #tweetObject = row[1]
 tweet = tweetObject#['new_tweet']
...
def inTweetANeut1(TeamA, tweetObject):
 #tweetObject = row[1]
 tweet = tweetObject#['new_tweet']
 for teamNames in TeamA:
  string_you_are_searching_for = r"\b" + str(teamNames) + r"\b"
...
def inTweetBNeut1(TeamB, tweetObject):
 #tweetObject = row[1]
 tweet = tweetObject#['new_tweet']
 for teamNames in TeamB:
  string_you_are_searching_for = r"\b" + str(teamNames) + r"\b"
...
def inTweetANeg1(TeamA, tweetObject):
 tweet = tweetObject#['new_tweet']
 hashtag = []
 #global TeamANeg
 global wordCloudNegA
...
def inTweetBNeg1(TeamB, tweetObject):
 #tweetObject = row[1]
 tweet = tweetObject#['new_tweet']
 hashtag = []
 #global TeamBNeg
...
def sent2(group, engl=True):
 for row in group[1:]:
  tweetObject = row[1]
  #print(type(tweetObject), tweetObject, "ROW IS OVER")
  ...
  #tweet = tweetObject#['new_tweet']
   ...
   #tweet = trans
  #tweet = tweetObject#['new_tweet']
  tweetObject = ''.join(tweetObject)
  print(tweetObject, "sent2 tweetObject")
  print("tweetObject over")

  score = analyser.polarity_scores(tweetObject)
  lb = score['compound']
  if lb >= 0.02:
   print("Positive")
   inTweetAPos1(TeamA, tweetObject)
   inTweetBPos1(TeamB, tweetObject)
   global pos 
  #pos = pos + 1

  elif (lb > -0.05) and (lb < 0.02):
   print("Neutral")
   inTweetANeut1(TeamA, tweetObject)
   inTweetBNeut1(TeamB, tweetObject)
   global neut
   #neut = neut + 1
    
  else:
   print("Negitive")
   inTweetANeg1(TeamA, tweetObject)
   inTweetBNeg1(TeamB, tweetObject)
   global neg
   #neg = neg + 1
   global tweetCount 
   tweetCount = tweetCount + 1 
  ...
  try:
   ...
   #print("pos",pos,"neg",neg,"neut",neut,"TeamAPosLive", TeamAPosLive,"TeamBPosLive", TeamBPosLive)
   print("TeamANeutLive",TeamANeutLive,"TeamBNeutLive",TeamBNeutLive,"TeamANegLive",TeamANegLive,"TeamBNegLive",TeamBNegLive)
...
def average(TeamAPosLive, TeamBPosLive, TeamANeutLive, TeamBNeutLive, TeamANegLive, TeamBNegLive): 
 try: 
  TeamATotalLive = (TeamAPosLive - TeamANegLive) //(TeamAPosLive + TeamANeutLive + TeamANegLive)
  print("printing average")
  return TeamATotalLive
  TeamBTotalLive = (TeamBPosLive - TeamBNegLive) //(TeamBPosLive + TeamBNeutLive + TeamBNegLive)
  print("printing average")
  return TeamBTotalLive 
 except ZeroDivisionError:
  print("Error")
...
def sorting(data):
 ...
 #df['Datetime'] = pd.to_datetime(df['2'])
 df = df.set_index(pd.DatetimeIndex(df[2]))
 #df.resample('T')
 #df.groupby(df.index.map(lambda t: t.hour, t.minute))
 #print(df)
 ...
 for group in grouped:
  ...
  #plothot.append(each)
 print(plothot, "IM printing plothot")
